...
class FormViewModel extends LoadingViewModel {
  ...
  final String TAG = "FormViewModel";
  ...
  PageInfo currentPage = PageInfo(forms: []);
  ...
  Map<String, WidgetInfo> datePickerFields = {};

  /// This map stores file picker form fields by their key
  Map<String, WidgetInfo> filePickerFields = {};
  ...
  findCurrentForm(String key) {
    if (formsList.forms.isNotEmpty && key.isNotEmpty) {
      for (FrameworkForm form in formsList.forms) {
        if (form.formKey == key) {
          currentForm = form;
          // if current page is UI type
          print(
              "CURRENT FORM IS: ${currentForm.formName} and type is ${currentForm.formType}");
          backstack.add(currentForm);
          break;
        }
      }
      for (PageInfo pageInfo in FormViewModel.forms) {
        if (pageInfo.pageId == key) {
  ...
  fieldButtonPressed(FrameworkFormField button, BuildContext context,
    ...
    if (button.label.toLowerCase().contains('forgot password')) {
      ...
      return;
    } else if (button.label.toLowerCase().contains(constants.peripheralsPage)) {
      NavigationUtil.instance.navigateToPrinterScreen(context);
      backstack.add(currentForm);
      return;
    }

    if (button.uiType == constants.printer && !button.valuesApi.isPreview) {
      final result = await showDialog<int>(
        context: context,
        builder: (BuildContext context) {
          return const CounterDialog();
        },
      );
      printLabel(context, button, result!);
      // return;
    }
      ...
      case 2:
        ...
        if (currentForm.formType.isNotEmpty &&
          ...
          if (button.valuesApi.isPreview) {
            ...
            formPreview.navBar = currentForm.navBar;
            formPreview.formKey = constants.previewScreenPage;
            ...
            if (currentForm.formType == constants.previewScreenPage) {
  ...
  nextButtonPressed(DecisionNode decisionNode, BuildContext context) {
    ...
    if (subform.isNotEmpty) {
      ...
      if (currentForm.formType == constants.landingPage) {
        ...
        if (constants.landingPage == currentForm.formType) {
          Navigator.of(context).pop('');
  ...
  onBackPressed(BuildContext context) {
    ...
    if (backstack.isEmpty || backstack.length == 1) {
      ...
      for (PageInfo pageInfo in FormViewModel.forms) {
        if (pageInfo.pageId == currentForm.formKey) {
          currentPage = pageInfo;
          break;
        }
      }
      ...
      if (cForm.formType == constants.previewScreenPage) {
        Navigator.pop(context);
      } else if (currentForm.formType == constants.landingPage) {
        Navigator.pop(context);
        Provider.of<HomeViewModel>(context, listen: false).notifyListeners();
      }
      notifyListeners();
  ...
  traverseSubForms(FrameworkForm form, BuildContext context) {
    ...
    PageInfo currentSubPage = PageInfo(forms: []);
    for (PageInfo pageInfo in FormViewModel.forms) {
      if (pageInfo.pageId == form.formKey) {
        currentSubPage = pageInfo;
        break;
      }
    }

    List<PageForm> forms = currentSubPage.forms;
    ...
    for (FrameworkFormField field in form.fields) {
      if (field.uiType == constants.row || field.uiType == constants.subform) {
        ...
        if (subformWidgets.isNotEmpty) {
          ...
          field.uiType == constants.geofence ||
        ...
        if (value.isNotEmpty) {
          outerLoop:
          for (PageForm pageForm in forms) {
            List<WidgetInfo>? widgetInfoList = pageForm.widgets;
            for (WidgetInfo widgetInfo in widgetInfoList) {
              if (field.key == widgetInfo.widgetId) {
  ...
  _addValuesToEntityInstances(FrameworkForm form) async {
    ...
    if (entityKey.isEmpty) {
      if (form.formType != constants.customPage) {
        return;
      }
    ...
    for (FrameworkFormField field in form.fields) {
      ...
      if (field.uiType == constants.edittext ||
        ...
        if (AppState.instance.formTempMap[field.key] != null) {
          String dateString = AppState.instance.formTempMap[field.key].toString();
          if (dateString.contains(',')) {
            ...
            DateTime v = DateTime.parse(dateString);
        ...
        value =
          ...
          field.uiType == constants.filePicker) {
        List<dynamic> mediaList = AppState.instance.formTempMap[field.key];
        if (mediaList.isNotEmpty) {
          List<FormImageFieldWidgetMedia> updatedMediaList = [];
          ...
          int uploadedCount = 0;
          String dialogText = "Uploading files of ${field.label}";
          for (FormImageFieldWidgetMedia media in mediaList) {
            int totalImage = mediaList.length;
            ...
            if (media.isLocal) {
              ...
              String mediaUrl =
                  await ImageUploadProvider().uploadMediaToS3(mediaFile);
              media.url = mediaUrl;
              if (media.thumbnailLocalPath != null) {
                File thumbnailFile = File(media.thumbnailLocalPath!);
                String thumbnailUrl =
                    await ImageUploadProvider().uploadMediaToS3(thumbnailFile);
                print("thumbnail updated : $thumbnailUrl");
          ...
          AppState.instance.formTempMap[field.key] = updatedMediaList;
  ...
  getTheme() async {
    final client = ApiService.getClient(constants.authBaseUrl);
    ThemeResult themeResult;
    try {
      themeResult = await client.fetchTheme();
      if (themeResult.statusCode != null &&
          themeResult.statusCode == 200 &&
          themeResult.response != null) {
        AppState.instance.setTheme(themeResult.response!.theme);
      } else if (themeResult.statusCode != null &&
          themeResult.statusCode == 401) {
        Util.instance.logMessage('Home Repo', 'Error while parsing theme');
      }
    } on dio.DioException catch (e) {
      Util.instance.logMessage(
          'FormViewModel: Dio Exception', 'Error while fetching theme -- $e');
    } catch (e) {
      Util.instance
          .logMessage('FormViewModel', 'Error while fetching theme -- $e');
    }
  }
  ...
  getAppBar(BuildContext context, {String? title, List<int>? iconList}) {
    ...
    Util.instance.logMessage(TAG, title ?? navTextField?.label ?? "");
  ...
  static List<PageInfo> forms = [];

  static void setFormPages(List<PageInfo> formsPages) {
    forms = formsPages;
  }

  Future<void> printLabel(
      BuildContext context, FrameworkFormField button, int noOfCopies) async {
    if (button.image.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text(constants.printerZPLMissing),
      ));
      return;
    }

    String zpl = button.image;

    List<String> list = zpl.split('{');
    List<String> zplList = <String>[];

    List<int> position = [];

    if (list.length <= 1) {
      await Printer(zpl).print(noOfCopies);
    }

    zpl = '';
    for (String part in list) {
      List<String> part2 = part.split('}');

      if (part2.length > 1) {
        if (part2[0].contains('FRT(') ||
            part2[0].contains('FLT(') ||
            part2[0].contains(',')) {
          List<String> dataList = _manipulateData(part2[0]);

          for (int i = 0; i < dataList.length; i++) {
            if (dataList[i].contains('::')) {
              position.add(zplList.length + i);
            }
          }
          zplList.addAll(dataList);
          zplList.add(part2[1]);
        } else {
          FormWidgetData? widgetData =
              AppState.instance.formTempWidgetMap[part2[0]];
          dynamic value = AppState.instance.formTempMap[part2[0]] ?? "";
          switch (widgetData?.field.uiType) {
            case constants.date:
              part2[0] = Util.instance.getDisplayDate(value);
              break;
            default:
              if (value.toString().contains('::')) {
                position.add(zplList.length);
              }
              part2[0] = value;
          }
          zplList.addAll(part2);
        }
      } else {
        zplList.addAll(part2);
      }
    ...
    if (position.isNotEmpty) {
      serialPrint(zplList, position, noOfCopies);
    } else {
      Printer(zplList.join()).print(noOfCopies);
    }
  }

  Future<void> serialPrint(
      List<String> zplList, List<int> position, int noOfCopies) async {
    List<String> range = zplList[position[0]].split('::');

    for (int i = int.parse(range[0]); i <= int.parse(range[1]); i++) {
      for (int j = 0; j < position.length; j++) {
        zplList[position[j]] = i.toString();
      }
      String zpl = zplList.join();
      await Printer(zpl).print(noOfCopies);
    }
  }

  List<String> _manipulateData(String value) {
    List<String> formulaList = value.split(',');
    List<String> resultList = <String>[];

    for (int i = 0; i < formulaList.length; i++) {
      String formula = formulaList[i];
      if (formula.startsWith('FLT(')) {
        List<String> dataList =
            formula.substring(4, formula.length - 1).split(':');
        dataList[0] = AppState.instance.formTempMap[dataList[0]];
        dataList[0] = dataList[0].substring(0, int.parse(dataList[1]));
        resultList.add(dataList[0]);
      } else if (formula.startsWith('FRT(')) {
        List<String> dataList =
            formula.substring(4, formula.length - 1).split(':');
        dataList[0] = AppState.instance.formTempMap[dataList[0]];
        dataList[0] = dataList[0].substring(
            dataList[0].length - int.parse(dataList[1]), dataList[0].length);
        resultList.add(dataList[0]);
      } else {
        formula = AppState.instance.formTempMap[formula];
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      content: Column(
        ...
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          const Center(child: Text("No. of copies: ")),
          Row(
            children: [
              IconButton(
      ...
      actions: [
        TextButton(
          ...
          child: Text('OK', style: TextStyle(color:  HexColor(AppState.instance.themeModel.primaryColor)),),
        ),
      ],
    );
  }
