...
class ValidationUtil {
  ...
  Map<String, String> validateForm(
    ...
    for (FrameworkFormField field in form.fields) {
      ...
      if (!isMandatory(field.validations) &&
          field.uiType == constants.dropdown) {
        ...
        if (isMandatory(field.validations) &&
          ...
          validationErrorMsg = getMandatoryMessage(field.validations);
          errorFieldMap[field.key] = validationErrorMsg;
        } else {
          validationErrorMsg = minMaxValidationError(field.validations, value);
          if (validationErrorMsg.isNotEmpty) {
            errorFieldMap[field.key] = validationErrorMsg;
          }
        }
      } else if (isMandatory(field.validations) &&
          field.uiType != constants.subform) {
  ...
  static bool isMandatory(List<FrameworkValidation> validations) {
    for (FrameworkValidation validation in validations) {
      if (validation.validationKey == 'req' &&
          validation.validationValue?.toLowerCase() == 'true') {
        return true;
      }
    }
    return false;
  ...
  static bool isMandatoryV2(List<Validations>? validationsList) {
    if (validationsList != null) {
      for (Validations validation in validationsList) {
        if (validation.validationKey == 'req' &&
            validation.validationValue?.toLowerCase() == 'true') {
          return true;
        }
  ...
  String getMandatoryMessage(List<FrameworkValidation> validations) {
    for (FrameworkValidation validation in validations) {
      if (validation.validationKey == 'req' &&
          validation.validationValue?.toLowerCase() == 'true') {
  ...
  String minMaxValidationError(List<FrameworkValidation> validations, String? value) {
    for (FrameworkValidation validation in validations) {
      num? valueNum = Util.instance.toNum(value!);
      if (valueNum != null) {
        if (validation.validationKey == 'min' && valueNum < int.parse(validation.validationValue!)) {
          return 'Value can not be less than ${validation.validationValue!}';
        } else if (validation.validationKey == 'max' && valueNum > int.parse(validation.validationValue!)) {
          return 'Value can not be greater than ${validation.validationValue!}';
        }
      } else {
        if (validation.validationKey == 'min' && value.length < int.parse(validation.validationValue!)) {
          return 'Length can not be less than ${validation.validationValue!}';
        } else if (validation.validationKey == 'max' && value.length > int.parse(validation.validationValue!)) {
          return 'Length can not be greater than ${validation.validationValue!}';
        }
      }
    }
    return '';
